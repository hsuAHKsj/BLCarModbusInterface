...
public:
    ...
    modbus(std::string host, int16_t port);
    ~modbus();
    ...
    int modbus_read_coils(int16_t address, int16_t amount, bool *buffer);
    int modbus_read_input_bits(int16_t address, int16_t amount, bool *buffer);
    int modbus_read_holding_registers(int16_t address, int16_t amount, int16_t *buffer);
    int modbus_read_input_registers(int16_t address, int16_t amount, int16_t *buffer);

    int modbus_write_coil(int16_t address, const bool &to_write);
    int modbus_write_register(int16_t address, const int16_t &value);
    int modbus_write_coils(int16_t address, int16_t amount, const bool *value);
    int modbus_write_registers(int16_t address, int16_t amount, const int16_t *value);
...
private:
    ...
    void modbus_build_request(uint8_t *to_send, int16_t address, int func) const;

    int modbus_read(int16_t address, int16_t amount, int func);
    int modbus_write(int16_t address, int16_t amount, int func, const int16_t *value);
 ...
 */
inline modbus::modbus(std::string host, int16_t port = 502)
{
    HOST = host;
    PORT = port;
    _slaveid = 1;
 ...
 */
inline void modbus::modbus_build_request(uint8_t *to_send, int16_t address, int func) const
{
 ...
 */
inline int modbus::modbus_write(int16_t address, int16_t amount, int func, const int16_t *value)
{
    int status = 0;
    uint8_t *to_send;
 ...
 * @param func      Modbus Functional Code
 */
inline int modbus::modbus_read(int16_t address, int16_t amount, int func)
{
    uint8_t to_send[12];
    modbus_build_request(to_send, address, func);
    to_send[5] = 6;
 ...
 */
inline int modbus::modbus_read_holding_registers(int16_t address, int16_t amount, int16_t *buffer)
{
    if (_connected)
    {
        ...
        {
            buffer[i] = ((int16_t)to_rec[9u + 2u * i]) << 8u;
            buffer[i] += (int16_t)to_rec[10u + 2u * i];
        }
        return 0;
    }
    else
 ...
 * @param buffer      Buffer to Store Data Read from Registers
 */
inline int modbus::modbus_read_input_registers(int16_t address, int16_t amount, int16_t *buffer)
{
    if (_connected)
    {
 ...
 */
inline int modbus::modbus_read_coils(int16_t address, int16_t amount, bool *buffer)
{
    if (_connected)
    {
        if (amount > 2040)
 ...
 */
inline int modbus::modbus_read_input_bits(int16_t address, int16_t amount, bool *buffer)
{
    if (_connected)
    {
        if (amount > 2040)
 ...
 */
inline int modbus::modbus_write_coil(int16_t address, const bool &to_write)
{
    if (_connected)
    {
        int value = to_write * 0xFF00;
        modbus_write(address, 1, WRITE_COIL, (int16_t *)&value);
        uint8_t to_rec[MAX_MSG_LENGTH];
        ssize_t k = modbus_receive(to_rec);
        if (k == -1)
        {
 ...
 */
inline int modbus::modbus_write_register(int16_t address, const int16_t &value)
{
    if (_connected)
    {
 ...
 */
inline int modbus::modbus_write_coils(int16_t address, int16_t amount, const bool *value)
{
    if (_connected)
    {
        int16_t *temp = new int16_t[amount];
        for (int i = 0; i < amount; i++)
        {
            temp[i] = (int16_t)value[i];
        }
 ...
 */
inline int modbus::modbus_write_registers(int16_t address, int16_t amount, const int16_t *value)
{
    if (_connected)
    {
